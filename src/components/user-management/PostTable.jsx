...
const PostTable = ({posts, page=1, totalPages, onPageChange:setPage}) => {
        ...
        <table className="w-full border-collapse border">
          <thead className="bg-[#0057FF] text-white">
            <tr className="text-left">
              <th className="p-4 border-t border-l border-r border-[#EBEBEB] font-semibold text-base">Report ID</th>
              <th className="p-4 border-t border-l border-r border-[#EBEBEB] font-semibold text-base">User ID</th>
              <th className="p-4 border-t border-l border-r border-[#EBEBEB] font-semibold text-base">Post Type</th>
              <th className="p-4 border-t border-l border-r border-[#EBEBEB] font-semibold text-base">Report</th>
              <th className="p-4 border-t border-l border-r border-[#EBEBEB] font-semibold text-base">Join Date</th>
              <th className="p-4 border-t border-l border-r border-[#EBEBEB] font-semibold text-base">Time Stamp</th>
            ...
            {posts.map((p, idx) => (
              <tr key={idx} className="border-b">
                <td className="p-5 border-b border-l border-r border-[#F5F5F5] font-semibold text-base text-[#2E2E2E] max-w-40 truncate">
                    <span >{p.requestId}</span>
                </td>
                <td className="p-5 border-b border-l border-r border-[#F5F5F5] font-semibold text-base text-[#2E2E2E]">
                  <div className="flex w-full items-center gap-2">
                    <span className="max-w-[80%] truncate">{p.userId}</span>
                    <span>{icons.crown}</span>
                  </div>
                </td>
                <td className="p-5 border-b border-l border-r border-[#F5F5F5] font-semibold text-base text-[#2E2E2E]">
                  <div className="flex">
                    <span className="max-w-40 truncate">{p.postType}</span>
                  </div>
                </td>
                <td className="p-5 border-b border-l border-r border-[#F5F5F5] font-semibold text-base text-[#2E2E2E]">
                  <div className="flex">
                    <span className="max-w-40 truncate">{p.report}</span>
                  </div>
                </td>
                <td className="p-5 border-b border-l border-r border-[#F5F5F5] font-semibold text-base text-[#2E2E2E]">{p.joinDate}</td>
                <td className="p-5 border-b border-l border-r border-[#F5F5F5] font-semibold text-base text-[#2E2E2E]">{p.timeStamp}</td>
                <td className="p-5 border-b border-l border-r border-[#F5F5F5] font-semibold text-base">
                  {handlePostStatus(p.status)}
                </td>
...
const handlePostStatus = (status)=>{
  if(status === 'Resolved') return (
  ...
  else if(status === 'Under review') return (
    ...
    <span className={`text-sm font-medium flex items-center gap-1 py-1 px-3 rounded-lg bg-[#FEFAFA] text-[#D61C2B]`}>
      {icons.deleted}
      {status}
    </span>
  )
...
const icons = {
  ...
  deleted: <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M14.7467 5.15301L12.0133 2.41967C11.6933 2.09967 11.0533 1.83301 10.6 1.83301H6.73333C6.28 1.83301 5.64 2.09967 5.32 2.41967L2.58667 5.15301C2.26667 5.47301 2 6.11301 2 6.56634V10.433C2 10.8863 2.26667 11.5263 2.58667 11.8463L5.32 14.5797C5.64 14.8997 6.28 15.1663 6.73333 15.1663H10.6C11.0533 15.1663 11.6933 14.8997 12.0133 14.5797L14.7467 11.8463C15.0667 11.5263 15.3333 10.8863 15.3333 10.433V6.56634C15.3333 6.11301 15.0667 5.47301 14.7467 5.15301ZM11.3533 10.4797C11.5467 10.673 11.5467 10.993 11.3533 11.1863C11.2533 11.2863 11.1267 11.333 11 11.333C10.8733 11.333 10.7467 11.2863 10.6467 11.1863L8.66667 9.20634L6.68667 11.1863C6.58667 11.2863 6.46 11.333 6.33333 11.333C6.20667 11.333 6.08 11.2863 5.98 11.1863C5.78667 10.993 5.78667 10.673 5.98 10.4797L7.96 8.49967L5.98 6.51967C5.78667 6.32634 5.78667 6.00634 5.98 5.81301C6.17333 5.61967 6.49333 5.61967 6.68667 5.81301L8.66667 7.79301L10.6467 5.81301C10.84 5.61967 11.16 5.61967 11.3533 5.81301C11.5467 6.00634 11.5467 6.32634 11.3533 6.51967L9.37333 8.49967L11.3533 10.4797Z" fill="#D61C2B"/>
</svg>
