...
const FlagsTable = ({reports=[], page=1, totalPages, setPage, itemsTabs, itemsActiveTab, setItemsActiveTab, search, setSearch}) => {
        ...
        {reports.length > 0 ? (
            <div>
                ...
                <thead className="bg-[#E9F1FF] text-[#1C5ECA]">
                    ...
                    <th className="p-4 border-t border-l border-r border-[#EBEBEB] font-semibold text-base">Flag ID</th>
                            ...
                            {openMenuId === r._id && (
                                ...
                                >
                                 ...
                                 {r.status !== 'investigating' && 
                                    <button
                                        onClick={async()=>{
                                            await toggleAppealStatus(r._id, 'investigating')
                                        }} 
                                        className="block w-full text-left px-4 py-2 text-sm text-[#1C357E] hover:bg-gray-50"
                                    >
                                ...
                                {r.status !== 'resolved' && 
                                    <button
                                    onClick={async()=>{
                                        await toggleAppealStatus(r._id, 'resolved')
                                    }} 
                                    className="block w-full text-left px-4 py-2 text-sm text-[#1C357E] hover:bg-gray-50"
                                    >
                                        Mark as resolved
                                ...
                                {r.status !== 'dismissed' && 
                                    <button
                                        onClick={async()=>{
                                            await toggleAppealStatus(r._id, 'dismissed')
                                        }} 
                                        className="block w-full text-left px-4 py-2 text-sm text-[#1C357E] hover:bg-gray-50"
                                    >
                                        Dismiss report
            ...
            <p className='text-center'>No Flags yet</p>}
        </>
    </div>
  )
}
