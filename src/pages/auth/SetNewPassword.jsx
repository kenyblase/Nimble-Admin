...
import { ChevronLeft, ChevronRightCircleIcon, Lock, User } from "lucide-react";
import { useSetNewPassword } from "../../utils/useApis/useAuthApis/useSetNewPassword";
...
const SetNewPassword = () => {
  ...
  const {setNewPassword, isSettingPassword} = useSetNewPassword()
  ...
  const handleSubmit = async(e) => {
    ...
    await setNewPassword({email: admin.email, password})
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="w-[35%] min-w-md p-10 rounded-lg shadow-2xl border border-gray-200 text-center">
        <Link to={'/login'} className="flex items-start gap-1 mb-10">
        ...
        <h1 className="text-2xl text-[#2E2E2E] font-semibold mb-1">Hi, {admin.fullName || 'Admin'}</h1>
        <p className="text-sm text-[#757575] font-semibold mb-6">Enter your preferred password to continue to dashboard</p>

        <div className="w-full bg-[#F5F8FE] border border-[#1C63F3] flex justify-center items-center gap-2 p-3 rounded-lg mb-5">
          <User color="#0D59F2"/>
          <p className="text-[#3B3B3B] font-semibold">{admin.email}</p>
        </div>
